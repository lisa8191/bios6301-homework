...
* Line 55, relying on column order works but it can be dangerous in practice
* Task 1.5, using a matrix coerced numeric values into character strings, which led to a bug in the next task
* Task 1.6, going from a matrix of type `character` to a data.frame coerced all values into factor variables.  Using `as.numeric` did not convert values back to numeric, but instead took the internal value.  While your thought process for calculating the difference is correct, the answers are wrong; -5
    * See that `as.numeric(as.factor(as.character(5)))` produces `1`
* Task 2.4, print functions should call `print`, not return data; -2
    * Try `print.data.frame(greaterzero)`
    * Making this modification would cause `plot`, `boxplot`, and `hist` to fail as they call `print`; a better solution would be to ensure `value` exists in `obj` for these functions
...
* Task 3.1, sample `nrow` values per position not 1 per position. Sorry this was unclear; -2
    * Use league object, calcpoints and buildValues to create values
